...
function main(params) {
  ...
  body = addLayerBands(body);
  body = addTopCover(body);
  body = addLugs(body);

  return [body.rotateX(-90).translate([0, 0, _d / 2])];//uncomment this line for production
  //return [body.translate([0,0,0])];//uncomment this line for development
  ...
  function addLayerBands(body) {
    ...
    for(var j=0; j<=params.house_h_count; j++){
      for(var i=0; i<numOfLayerBands; i++){
        body=union(body, vBand.translate([j*(_w-params.house_thickness),-(i+1)*bandOffset, 0]));
      }
    }

    var hBand = cube({size: [_sw, layerBandHeight, thick], center: true}).translate([0,_d/2, -_sh/2 + params.house_thickness/2]);
    for(var j=0; j<=params.house_v_count-1; j++){
      for(var i=0; i<numOfLayerBands; i++){
        body=union(body, hBand.translate([0, -(i+1)*bandOffset, j*(_h-params.house_thickness)]));
      }
    }
...
function getParameterDefinitions() {
  ...
  return [
    ...
    // },
    {
      ...
      type: 'float',
      initial: (window['g_prm_house_layer_band_height']!==undefined ? g_prm_house_layer_band_height : 2),
      step: 0.1,
      min: 1,
      max: 5,
      caption: 'Layer band height (1mm - 6mm):'
    },
    {
      name: 'house_top_lug',
      type: 'checkbox',
